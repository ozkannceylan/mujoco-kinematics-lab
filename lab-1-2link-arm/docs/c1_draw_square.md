...
|--------|--------------|---------|
| A2 (`src/a2_forward_kinematics.py`) | FK reference geometry | Path sanity checks and workspace reasoning |
| A3 (`src/a3_jacobian.py`) | `analytic_jacobian` | J⁻¹ velocity mapping |
| A4 (`src/a4_inverse_kinematics.py`) | `analytic_ik` | Cartesian → joint-space |
| A5 (MuJoCo runtime) | `mj_fullM`, `data.qfrc_bias` | M(q), bias terms |
| B1 (`src/b1_trajectory_generation.py`) | `quintic_profile` | Smooth Cartesian path |
